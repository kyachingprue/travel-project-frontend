...
const ShortTourCard = () => {
  ...
  return (
    <div>
      <h3 className='text-white py-5 text-center text-2xl md:text-4xl font-bold'>This is Short Page ({tour.length})</h3>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 py-10 gap-3'>
        {
          tour.slice(0, 8).map(item => <TourCard key={item.id} item={item}></TourCard>)
        }
      </div>
      <Link to="/packages">
        <button className="
